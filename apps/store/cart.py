    ...
    def __iter__(self):
        ...
        # Check if the product ID exists in the cart
            if str(product.id) in cart:
                # Update the corresponding entry in the copied cart dictionary with the product details
                cart[str(product.id)]['product'] = product
                
                # Calculate the total price for each item by multiplying the product price by the quantity
